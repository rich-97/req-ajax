...
const html = fs.readFileSync('example/index.html')
...
function onRequest (req, res) {
  ...
  if (reqUrl.pathname === '/') {
    ...
    res.writeHead(200)
    res.end()
  } else if (reqUrl.pathname === '/example.json') {
    let json = fs.readFileSync('example/example.json')
    res.writeHead(200)
    ...
    res.writeHead(200)
    res.end()
  } else if (reqUrl.pathname === '/example.js') {
    let js = fs.readFileSync('example/example.js')
    res.write(js)
    res.end()
  } else if (reqUrl.pathname === '/ajax.js') {
    let ajax = fs.readFileSync('dist/ajax.js')
    res.write(ajax)
    res.end()
  }
}
...
function onListen () {
  console.log(`The server test is running on http://localhost:${port}.`)
}
